        ...
        <div className="flex flex-col gap-4">
          <div className="flex justify-between">
            <div className="flex items-center gap-2">
              ...
              <img src={smiley} className="w-8" alt="" />
            </div>
            ...
            <div>
              {UserScaleData.favorites.length === 0 ? (
                <div className="w-full flex justify-center items-center gap-3 flex-col md:flex-row">
                  <img src={NoFavorites} className="w-40 object-cover rounded-full h-40" alt="" />
                  <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  {UserScaleData
                    ? UserScaleData.favorites.map((item, index) => (
                        <div key={index}>
                          <ScaleCard scaleName={item.scaleName} scaleImage={item.scaleImg} scaleDescription={item.scaleDescription} onView={() => handleViewDetails(item)} />
                        </div>
                      ))
                    : null}
                </div>
              ...
              {isModalOpen && (
                <ScaleViewModal onClose={handleCloseModal} className="z-30">
                  ...
                  <p className="font-bold font-montserrat tracking-tight mt-2 ">
                    Type: <span className="font-normal font-poppins text-[15px]">{selectedScale.scaleType}</span>
                  </p>
        ...
        <div className="my-20">
          <div className="flex justify-between items-center flex-wrap">
            ...
            <div className="py-4 md:px-6 px-[9px] rounded-xl bg-white flex items-center gap-2  md:gap-3 relative z-10">
              ...
              {/* Date */}
              <div className="flex gap-3 items-center pr-3 border-r-[2px] border-gray-300 cursor-pointer" onClick={handleOpenSelect}>
                ...
                {openScale && (
                  <div className="bg-white rounded-xl shadow-md p-4 md:w-50 w-30 flex flex-col gap-1">
                    {UserScaleData.scales.map((item, index) => (
                      <div key={index}>
                        <p className="text-[15px] font-poppins tracking-tight uppercase cursor-pointer hover:text-dowellDeepGreen hover:scale-105 transition-all ease-in-out duration-100 hover:font-semibold" onClick={() => (filterByName(item.scaleName), setOpenScale(false))}>
                          {item.scaleName}
                        </p>
                      </div>
                    ))}
              ...
              <div className="flex gap-1 md:gap-3 items-center cursor-pointer" onClick={handleResetFilter}>
                <BiReset className="md:size-6 size-4  text-red-500" onClick={handleResetFilter} />
          ...
          <div className="my-8 rounded-md">
            {filteredScales.map((item, index) => (
              <div key={index} className="bg-white mb-2 px-6 py-4 rounded-xl cursor-pointer">
