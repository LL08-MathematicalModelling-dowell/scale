...
const LikertScale = () => {
  ...
  const handleClick = async (index) => {
    ...
    try {
      const response = await scaleResponse(false, scaleType, channel, instance, workspace_id, username, scale_id, submitted);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert("Your response was submitted, but we couldn't send the email. Please try again.");
    }
  ...
  return (
    <div className="flex items-center justify-center min-h-screen p-4 overflow-x-hidden">
      <div className="flex flex-col items-center w-full max-w-full p-4 rounded-lg bg-card md:max-w-lg">
            ...
            <div className="relative" key={index}>
              <img
                ...
                className={`cursor-pointer 
                                        ${submitted === index + 1 ? `border-2 ${submitted == 1 ? "border-red-500" : submitted == 2 ? "border-orange-500" : submitted == 3 ? "border-yellow-400" : submitted == 4 ? "border-[#acd91a]" : "border-green-700"} rounded` : ""}`}
                alt={`emoji-${index + 1}`}
              />
            </div>
          ))}
        ...
        <p className="mt-0 mb-8 text-sm text-center text-muted-foreground md:text-base">Select your response</p>
        ...
        <textarea className="w-full h-20 p-2 mb-2 border border-gray-400 border-border" placeholder="Your Comments (Optional)" aria-label="Your Comments" value={feedback} onChange={(e) => setFeedback(e.target.value)} />
        <div className="flex w-full">
          ...
          <div className="w-[40%] flex flex-col md:items-center justify-center md:space-y-2 mb-2 w-full">
            <input type="text" className="w-full p-2 mb-2 border border-gray-400 border-border" placeholder="Your Name (Optional)" aria-label="Your Name" value={name} onChange={(e) => setName(e.target.value)} />
            <input type="email" className="w-full p-2 border border-gray-400 border-border" placeholder="Your Email (Optional)" aria-label="Your Email" value={email} onChange={(e) => setEmail(e.target.value)} />
            <div className="flex justify-center w-full mt-4 space-x-4">
              <button className="bg-[#9390a0] text-[#fff] py-1 px-4 rounded-full w-full md:w-auto" onClick={handleCancel}>
                Cancel
              </button>
              <button
        ...
        <div className="flex flex-row items-center justify-between w-full">
          ...
          <footer className="text-sm text-center text-muted-foreground">
            ...
            <a href="mailto:dowell@dowellresearch.sg" className="text-[#5f5f5f] text-xs md:text-sm">
