...
import {MenuItem, CircularProgress, LinearProgress, Grid, Typography, Box} from "@mui/material";
import axios, {all} from "axios";
import {Chart as ChartJS, CategoryScale, LinearScale, LineElement, PointElement, Title, Tooltip, Legend} from "chart.js";
import {Line} from "react-chartjs-2";
import {isArray} from "chart.js/helpers";
import {getUpdatedValues, pickSevenKeys, processData, filterDataWithinDays, transformData, deepClone, scorePercentagesfunc} from "../../../helper/jsFunctions";
import {deafultDatasets, defaultOptions, defaultLabels, defaultScorePercentages, defaultLearningDataIndex} from "../../../helper/jsObjects";
import LLXNavbar from "../LLXNavBar/LLXNavBar";
import {decodeToken} from "@/utils/tokenUtils";
import {getUserScales} from "../../../services/api.services";
...
const LLXReport = () => {
  ...
  useEffect(() => {
    ...
    helperChannel.map((channelName) => {
      responseData.map((data) => {
        if (data.channel == channelName.value) channelData.push(data);
      });
    });
    const filteredData = [];
    channelData.map((data) => {
      helperInstance.map((instanceName) => {
        if (data.instance.trim() == instanceName.value) filteredData.push(data);
      });
    });
    ...
    let selectedDaysCounts = arrayToObject(daysHelper),
      selectedDaysPercentages = {};
    ...
    if (llx < 1) setLearningStage(" Learning");
    else setLearningStage("Innovating");

    let labels, datasetsInfo, options, percentagesInfo, daysInfo, attendenceInfo, learningOptions;
    if (!objectPair || !arr || arr.length == 0) {
      (labels = defaultLabels), (datasetsInfo = deafultDatasets), (options = defaultOptions);
    } else {
  ...
  useEffect(() => {
    ...
    const allData = instanceList.flatMap((instance) => {
      return channelList.map((channel) => {
        const dataForInstanceAndChannel = responseData.filter((item) => item.instance.trim() === instance.value && item.channel.trim() === channel.value);
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(`https://www.scales.uxlivinglab.online/api/v1/learning-index-report/?scale_id=${scale_id}`);
      ...
      const uniqueInstances = Array.from(new Set(data.map((item) => item.instance.trim())));
  ...
  const questionData = [
    "Do you need more reading or explanation on the topic?",
  ...
  return (
    <Box p={1}>
      ...
      <Grid container spacing={3} alignItems="center" justifyContent="center">
        ...
        <Grid item xs={12} md={4}>
          <Select options={instanceOptionList} placeholder="Select Subject" value={selectedInstance} onChange={handleInstanceSelect} isSearchable={true} isMulti={enableMultiInstance} isDisabled={disableInstance} />
        </Grid>

        <Grid item xs={12} md={3}>
          <Select options={daysOptionList} placeholder="Select Days" value={daysOptions} onChange={handleDaysSelect} />
        </Grid>
      </Grid>
            ...
            displayDataForAllSelection.map((item, index) => {
              return (
                <>
                  ...
                  <p className="text-orange-600 font-bold text-[16px] ml-5 mt-5 mb-5 sm:mb-0">Learning funnel:</p>
        ...
        <>
          ...
          <Grid container spacing={10} alignItems="center" justifyContent="center">
            ...
            <Grid
              item
              xs={12}
              xl={!selectedChannel || !selectedInstance ? 10 : 7}
              sx={{
                mt: {xs: 0, md: 5},
              }}
            >
