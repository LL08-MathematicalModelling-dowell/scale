...
const LLXScaleDetails = () => {
  ...
  const handleButtonClick = async () => {
    ...
    try {
      ...
      const response = currentScaleType === "nps" ? await saveScaleDetails({hardCodedData, accessToken}) : await saveScaleDetailsType({hardCodedData, accessToken});
  ...
  return (
    <div className="max-h-screen max-w-full relative">
      ...
      <div className="flex justify-end p-6 items-center gap-4">
        ...
        <select value={selectedScaleType} onChange={handleScaleTypeChange} className="p-2 border rounded">
      ...
      <div className="flex-col flex">
        <div className="flex mt-2 flex-col md:flex-row flex-wrap md:gap-4 p-6 gap-4">
          {loading ? (
            ...
            <p className="w-full text-center text-lg font-semibold text-red-600 p-4">No scale found. Please create a scale for yourself.</p>
          ) : (
                  ...
                  <div className="flex-col flex gap-2">
                    ...
                    <div className="flex flex-col md:flex-row gap-6 flex-wrap">
                      {qrCode.links_details?.map((link, idx) => (
                        <QRCodeCard key={idx} imageSrc={link.qrcode_image_url} instanceName={getInstanceDisplayName(link.scale_link)} scaleLink={link.scale_link} qrCodeLink={qrCode.report_link.qrcode_image_url} type="scale" />
                      ))}
                    </div>
                  </div>
                  <div className="flex-col flex gap-2">
                    ...
                    <div className="flex flex-col md:flex-row gap-6">
                      <QRCodeCard imageSrc={qrCode.report_link.qrcode_image_url} instanceName={qrCode.username} type="report" qrCodeLink={qrCode.report_link.qrcode_image_url} reportLink={qrCode.report_link.report_link} />
                    </div>
                  </div>
                  <div className="flex-col flex gap-2">
                    <h1 className="text-[25px] font-bold font-poppins">Login Details</h1>
                    <div className="flex flex-col md:flex-row gap-6">{qrCode.login && <QRCodeCard imageSrc={qrCode.login.qrcode_image_url} instanceName={qrCode.username} type="login" qrCodeLink={qrCode.login.qrcode_image_url} loginLink={qrCode.login.login_link} />}</div>
                  </div>
                </div>
              ))}
              <button onClick={handleClick} className="bg-deepblue text-white shadow-xl px-2 py-2 rounded-full mt-4 md:mt-0 fixed md:bottom-[90px] bottom-[50px] md:right-12 right-8 z-10">
                <FaCirclePlus className="text-xl" />
              </button>
              {isModalOpen && (
                <div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex justify-center items-center z-50">
                  <div className="bg-white rounded-lg shadow-2xl p-8 w-full max-w-lg mx-4 md:mx-0">
                    <h2 className="text-xl font-semibold text-gray-700 mb-6 text-center">Select Scale Type</h2>
                    <div className="mb-6">
                      <label htmlFor="scaleType" className="block text-sm font-medium text-gray-600 mb-2">
                        Scale Type
                      </label>
                      <select id="scaleType" value={currentScaleType} onChange={handleScaleChange} className="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                    ...
                    <div className="flex justify-between items-center">
                      <button onClick={handleButtonClick} className="bg-indigo-600 text-white px-5 py-2 rounded-md shadow-lg hover:bg-indigo-700 transition-colors">
      ...
      {alert && (
        <div className="fixed top-24 right-4 flex items-center p-4 mb-4 text-sm text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400" role="alert" style={{zIndex: 9999}}>
...
export default LLXScaleDetails;
