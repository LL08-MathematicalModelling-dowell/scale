...
def format_llx_scale_data(data):
    table_data = []
    for entry in data:
        table_data.append([
            entry['date'],
            ...
            entry['average_score'],
            entry['total_responses'],
            entry['learning_level_stage']
        ])
    return tabulate(table_data, headers=['Date', 'Time Range', 'Total Score', 'Average Score', 'Total Responses', 'Learning Stage'], tablefmt='html')
...
def fetch_and_format_llx_scores(scale_id):
    ist = pytz.timezone('Asia/Kolkata')
    start_time = datetime.now(ist).replace(hour=0, minute=0, second=0, microsecond=0)
    end_time = start_time.replace(hour=21)
